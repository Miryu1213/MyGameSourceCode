...
#include <complex>
#include <algorithm>
//メモ　totaltime-全体の移動時間 time-0～1の間、進行度。min-開始の値（ほぼ0固定）。max-開始と終了の差分(ほぼ1固定)
//指数関数の緩やかさを設定できる
// イージング関数
struct Easing
{
...
public:
    ...
    template<class Ty>
    [[nodiscard]] static inline Ty RadIn2PI(Ty rad)
    {
...
public:
    ...
    [[nodiscard]] double OutExpDouble(double time, double totaltime)
    {
        double max = 1;
        double min = 0;
        max -= min;

        return AdjEqual<double>(time, totaltime) ?
            max + min :
            max * (-std::pow(_2<double>, -_10<double> * time / totaltime) + _1<double>) + min;
    }
