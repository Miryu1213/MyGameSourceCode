...
bool Signal::Update(Vector2 camPos, Vector2 camOffset)
{
	if (loadingTime_ <= 0)
	{
		return true;
...
{
	//float harfRad = Easing::ToRadian(TRIANGLE_DEGREE) / 2;

	////逆方向
	////角度
	//auto rdAngle = Easing::RadIn2PI(angleRad_ - harfRad);
	////正規化方向
	//Vector2Flt rdNormVec = { cos(rdAngle),sin(rdAngle) };
	////座標
	//Vector2Flt rightPos = pos_ - rdNormVec * size_;

	////正方向
	////角度
	//auto ldAngle = Easing::RadIn2PI(angleRad_ + harfRad);
	////正規化方向
	//Vector2Flt ldNormVec = { cos(ldAngle),sin(ldAngle) };
	////座標
	//Vector2Flt leftPos = pos_ - ldNormVec * size_;

	////Xが小さい順に記述する
	//std::vector<Vector2Flt> poses;
	//poses.push_back(pos_);
	//poses.push_back(rightPos);
	//poses.push_back(leftPos);

	//std::sort(poses.begin(), poses.end(), [](Vector2Flt& obj1, Vector2Flt& obj2) { return obj1.x_ < obj2.x_; });
